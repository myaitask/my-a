...
export const SENDER_EMAIL = process.env.RESEND_FROM_EMAIL || "team@www.myaitask.com";
export const SENDER_NAME = process.env.RESEND_FROM_NAME || "My AI Task";
export const REPLY_TO = process.env.RESEND_REPLY_TO || "myaitasks@gmail.com";
...
export async function sendWelcomeEmail(
  ...
  try {
    const data = await resend.emails.send({
      ...
      to: [toEmail],
      replyTo: REPLY_TO,
      subject: "Welcome to My AI Task — Your Social Media Automation Engine",
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #090d16; color: #f8fafc; padding: 40px 20px; text-align: center;">
          <div style="max-width: 540px; margin: 0 auto; background-color: #0f172a; border: 1px solid #1e293b; border-radius: 20px; padding: 36px 30px; text-align: left;">
            ...
            <div style="background-color: #1e293b; border-radius: 12px; padding: 18px; margin-bottom: 28px;">
              ...
              <a href="https://automate.myaitask.com" style="font-size: 14px; font-family: monospace; color: #818cf8; text-decoration: none;">
                automate.myaitask.com
              </a>
            </div>
            <a href="https://automate.myaitask.com" style="display: block; text-align: center; background-color: #4f46e5; color: #ffffff; font-weight: 600; font-size: 14px; padding: 14px 24px; border-radius: 12px; text-decoration: none;">
            ...
            <div style="margin-top: 32px; padding-top: 20px; border-top: 1px solid #1e293b; font-size: 11px; color: #64748b; text-align: center;">
              Sent by My AI Task • Dedicated Agency Automation Engine
            </div>
          </div>
        </div>
      `,
      ...
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #090d16; color: #f8fafc; padding: 40px 20px;">
          <div style="max-width: 500px; margin: 0 auto; background-color: #0f172a; border: 1px solid #1e293b; border-radius: 16px; padding: 28px;">
            ...
            <a href="https://automate.myaitask.com/crm" style="display: inline-block; background-color: #4f46e5; color: #ffffff; font-size: 13px; font-weight: 600; padding: 10px 18px; border-radius: 10px; text-decoration: none; margin-top: 10px;">
              View Lead in CRM →
            </a>
          </div>
        </div>
